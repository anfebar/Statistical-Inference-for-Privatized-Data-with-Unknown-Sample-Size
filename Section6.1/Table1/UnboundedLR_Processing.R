...
setwd("~/Downloads/outputs/")
#file_path = paste("./outputs/eps1_ep_N1_iteration",1,".RData",sep="")
#load(file_path)
burn_in = 5000
ep_vec = c(.1,1)
...
for(ep in ep_vec){
  ...
  Beta1_M = rep(NaN,length(N_DP_vec))
  Beta1_M_SE = rep(NaN,length(N_DP_vec))
  Beta1_V = rep(NaN,length(N_DP_vec))
  Beta1_V_SE = rep(NaN,length(N_DP_vec))      
  
  Beta2_M = rep(NaN,length(N_DP_vec))
  Beta2_M_SE = rep(NaN,length(N_DP_vec))
  Beta2_V = rep(NaN,length(N_DP_vec))
  Beta2_V_SE = rep(NaN,length(N_DP_vec))      
  
  Beta3_M = rep(NaN,length(N_DP_vec))
  Beta3_M_SE = rep(NaN,length(N_DP_vec))
  Beta3_V = rep(NaN,length(N_DP_vec))
  Beta3_V_SE = rep(NaN,length(N_DP_vec))      
  ...
  for(e in 1:length(N_DP_vec)){
    ...
    Beta1_M[e] = mean(beta_means[1,],na.rm=TRUE)
    Beta1_M_SE[e] = sqrt(var(beta_means[1,],na.rm=TRUE)/length(n_means))### monte carlo standard errors
    Beta1_V[e] = mean(beta_vars[1,],na.rm=TRUE)
    Beta1_V_SE[e] = sqrt(var(beta_vars[1,],na.rm=TRUE)/length(n_means))
    
    Beta2_M[e] = mean(beta_means[2,],na.rm=TRUE)
    Beta2_M_SE[e] = sqrt(var(beta_means[2,],na.rm=TRUE)/length(n_means))### monte carlo standard errors
    Beta2_V[e] = mean(beta_vars[2,],na.rm=TRUE)
    Beta2_V_SE[e] = sqrt(var(beta_vars[2,],na.rm=TRUE)/length(n_means))
    
    Beta3_M[e] = mean(beta_means[3,],na.rm=TRUE)
    Beta3_M_SE[e] = sqrt(var(beta_means[3,],na.rm=TRUE)/length(n_means))### monte carlo standard errors
    Beta3_V[e] = mean(beta_vars[3,],na.rm=TRUE)
    Beta3_V_SE[e] = sqrt(var(beta_vars[3,],na.rm=TRUE)/length(n_means))
  ...
  df = as.data.frame(round(1000*rbind(Beta1_M, Beta1_V, Beta2_M, Beta2_V, Beta3_M, 
                                      Beta3_V, Tau_M, Tau_V, N_M, N_V))/1000, 
  ...
  df_SE = as.data.frame(round(1000*rbind(Beta1_M_SE, Beta1_V_SE, Beta2_M_SE, 
                                         Beta2_V_SE, Beta3_M_SE, Beta3_V_SE, 
                                         Tau_M_SE, Tau_V_SE, N_M_SE, N_V_SE))/1000, 
                        stringsAsFactors = FALSE)
