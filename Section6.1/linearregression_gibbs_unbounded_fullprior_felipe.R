...
############################################################
###  Simulations originally ran at the High-performance computing (HPC) cluster
# id <- commandArgs(TRUE)
# cat("starting: ", id[1], "\n")
# iteration <- as.numeric(id[1])
for(iteration in 1:200)
{
  if(iteration <= 100) eps = 1
  if(iteration > 100){eps = 0.1; iteration = iteration - 100}
  eps
  iteration
  ...
  for(ep_N in c(10^7,1,0.1,0.01,0.001,0.0001))
  {
    ...
    if(!file.exists(paste("outputs/eps", privacy_params$eps, 
                          "_ep_N", privacy_params$ep_N,
                          "_iteration",iteration, ".RData", sep = "")))
    {
      chain = run_chain(iteration=iteration,num_cycles=num_cycles,N=N,N_guess=NaN,
                        hyperparams=hyperparams,
                        privacy_params = privacy_params,
                        swaps=1,jumps=1,fullUpdate=TRUE)
      
      save(num_cycles,N,hyperparams,privacy_params,chain,
           file = paste("outputs/eps", privacy_params$eps, 
                        "_ep_N", privacy_params$ep_N,
                        "_iteration",iteration, ".RData", sep = ""))
    }
    ...
    if(ep_N == 10^7)
    {
      if(!file.exists(paste("outputs/eps", privacy_params$eps, 
                            ...
                            "_iteration",iteration, ".RData", sep = "")))
      {
        chain = run_chain(iteration=iteration,num_cycles=num_cycles,N=N,N_guess=N,
                          hyperparams=hyperparams,
                          privacy_params = privacy_params,
                          swaps=1,jumps=0,fullUpdate=TRUE)
